...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	try {
		...
		const { personaId, query, conversationId, maxTokens } = result.data;
				...
				if (apiErrorMatch) {
					...
					if (details.includes('guardrail restrictions') || details.includes('data policy')) {
						return json({
							success: false,
							error:
								'OpenRouter: No endpoints available with current privacy settings. Configure at: https://openrouter.ai/settings/privacy'
						});
					}
					...
					if (details.includes('Insufficient balance') || details.includes('no resource package')) {
						return json({
							success: false,
							error:
								'ZAI API: Insufficient balance. Recharge at: https://z.ai/manage-apikey/billing'
						});
					}
