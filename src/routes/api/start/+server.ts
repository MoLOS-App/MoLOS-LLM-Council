...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  try {
    ...
    const selectedPersonas = await personaRepo.getByIds(
      personaIds || [],
      userId,
    );
    ...
    for (const persona of selectedPersonas) {
      if (!persona.provider?.apiToken) {
        console.error(
          "[Council] Persona missing provider/API token:",
          persona.name,
        );
        throw error(
          400,
    ...
    if (err instanceof Error) {
      ...
      if (err.message.includes("API error")) {
        ...
        if (apiErrorMatch) {
          ...
          // Check for ZAI coding endpoint being used
          if (
            details.includes("/coding/paas/v4") ||
            details.includes('path":"/v4"')
          ) {
            console.error(
          ...
          // Check for ZAI specific errors
            ...
            errorResponse = {
              success: false,
              error:
                "Invalid or expired ZAI API token. Please check your provider credentials.",
            };
          }
          // Provide user-friendly messages based on error type
            ...
            console.error(
              "[Council] API endpoint not found. Check provider configuration.",
            );
            errorResponse = {
              success: false,
              error:
                "Invalid provider API URL. Please check your provider settings.",
            };
          } else if (
      ...
      // Check for missing API key or URL
        ...
        errorResponse = {
          success: false,
          error:
            "Provider not configured. Please check your provider API key and URL in Settings.",
        };
      }
      // Other errors
      else {
