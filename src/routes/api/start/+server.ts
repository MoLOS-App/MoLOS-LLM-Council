...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	try {
		...
		console.log('[Council] Starting council with:', { query, personaIds, presidentPersonaId });
		...
		for (const persona of selectedPersonas) {
			...
			if (
				...
				console.error('[Council] ZAI general provider configured with coding endpoint');
				throw error(
					400,
			...
			if (
				...
				console.warn('[Council] ZAI coding provider configured with general endpoint');
			}
		...
		if (
			...
			console.error('[Council] President ZAI general provider configured with coding endpoint');
			throw error(
				400,
				'Invalid ZAI API URL. Please change provider type to "Z.AI (Coding)" or update API URL to: https://api.z.ai/api/paas/v4'
			);
		...
		if (
			...
			console.warn('[Council] President ZAI coding provider configured with general endpoint');
		}
 ...
 */
async function runCouncilSync(query: string, personas: any[], presidentPersona: any | null) {
	...
	const stage1Promises = personas.map(async (persona) => {
		try {
			...
			console.error(`[Council] Stage 1 failed for persona ${persona.name}:`, error);
			throw error;
		}
	});
	...
	const stage2Promises = personas.map(async (persona) => {
		try {
			...
			console.error(`[Council] Stage 2 failed for persona ${persona.name}:`, error);
			throw error;
		}
	});
	...
	if (presidentPersona && presidentPersona.provider?.apiToken) {
		try {
			...
			console.error(`[Council] Stage 3 failed for president ${presidentPersona.name}:`, error);
			throw error;
		}
	}
