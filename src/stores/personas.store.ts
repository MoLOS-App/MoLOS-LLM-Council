...
async function apiFetch<T>(path: string, options?: RequestInit, fetchFn?: typeof fetch): Promise<T> {
...
export async function createPersona(
	...
	const data = await apiFetch<{ persona: PersonaWithProvider }>('/personas', {
		method: 'POST',
...
export async function updatePersona(
	...
	const data = await apiFetch<{ persona: PersonaWithProvider }>(`/personas/${id}`, {
		method: 'PUT',
...
export async function createProvider(
	...
	const data = await apiFetch<{ provider: AIProvider }>('/providers', {
		method: 'POST',
...
export async function updateProvider(
	...
	const data = await apiFetch<{ provider: AIProvider }>(`/providers/${id}`, {
		method: 'PUT',
		body: JSON.stringify(updates)
	}, fetchFn);
...
export async function setDefaultProvider(id: string, fetchFn?: typeof fetch): Promise<void> {
	...
	providersStore.update((current) =>
		current.map((p) => ({ ...p, isDefault: p.id === id }))
	);
}
