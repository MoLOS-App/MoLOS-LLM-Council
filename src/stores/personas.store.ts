...
export async function loadPersonas(
  ...
  const data = await apiFetch<{ personas: PersonaWithProvider[] }>(
    "/personas",
    undefined,
    fetchFn,
  );
...
export async function loadProviders(
  ...
  const data = await apiFetch<{ providers: AIProvider[] }>(
    "/providers",
    undefined,
    fetchFn,
  );
...
export async function updatePersona(
  ...
  personasStore.update((current) =>
    current.map((p) => (p.id === id ? data.persona : p)),
  );
  return data.persona;
}
...
export async function updateProvider(
  ...
  providersStore.update((current) =>
    current.map((p) => (p.id === id ? data.provider : p)),
  );
  return data.provider;
}
