	...
	return [
		...
		// ============== PERSONAS ==============
		{
			...
			execute: async (params) => {
				if (params.presidentsOnly) {
					return await personaRepo.getByUserId(userId, { presidentsOnly: true });
				}
		...
		},
		{
			...
			parameters: {
				...
				properties: {
					...
					providerId: { type: 'string', description: 'ID of the AI provider to use' },
					isPresident: {
						type: 'boolean',
		...
		},
		{
			...
			parameters: {
				...
				properties: {
					id: { type: 'string', description: 'Conversation ID' },
					decisionSummary: { type: 'string', description: 'Final decision or summary' },
					tags: {
						type: 'array',
						items: { type: 'string' },
