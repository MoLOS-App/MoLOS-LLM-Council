...
async function selectPersonasForCouncil(
	...
	const presidentName = presidentPersona ? `${presidentPersona.avatar} ${presidentPersona.name}` : null;
...
async function runCouncilWithTimeout(
	...
	const timeoutPromise = new Promise<never>((_, reject) => {
		setTimeout(() => {
			reject(new Error(`Council execution timed out after ${Math.round(timeoutMs / 1000 / 60)} minutes. Try reducing the number of personas or using faster models.`));
		}, timeoutMs);
	});
    ...
    // ============== COUNCIL EXECUTION ==============
    {
      ...
      execute: async (params) => {
        try {
          ...
          const presidentPersonaId = params.presidentPersonaId as
           	| string
           	| undefined;
          ...
          let selectionInfo: { usedDefaults: boolean; selectedMembers: string[]; selectedPresident: string | null; message: string };
          ...
          if (selectedPersonas.length === 0) {
            return {
              error:
                "No personas available. Please create personas in Settings first.",
              success: false,
            };
          }
    ...
    },
    {
      ...
      execute: async () => {
        try {
          const allPersonas = await personaRepo.getByUserId(userId);
          const presidents = allPersonas.filter((p) => p.isPresident);
          const defaultMembers = allPersonas.filter(
            (p) => !p.isPresident && p.isDefault,
          );
          const nonDefaultMembers = allPersonas.filter(
            (p) => !p.isPresident && !p.isDefault,
          );
          ...
          return {
            ...
            selectionExplanation: defaultMembers.length > 0
