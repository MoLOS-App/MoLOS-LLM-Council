...
export class ProviderRepository {
	async getByUserId(userId: string): Promise<AIProvider[]> {
		const result = await db.select().from(councilProviders).where(eq(councilProviders.userId, userId));
	...
	async update(id: string, userId: string, updates: Partial<AIProvider>): Promise<AIProvider | null> {
		const result = await db
