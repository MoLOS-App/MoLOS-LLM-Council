...
export class SettingsRepository {
	...
	private mapToSettings(row: Record<string, unknown>): CouncilSettings {
		const defaultModelsJson = row.defaultModels as string;
		const defaultModels = defaultModelsJson ? JSON.parse(defaultModelsJson) : DEFAULT_SETTINGS.defaultModels!;

		return {
			...row,
			openrouterApiKey: (row.openrouterApiKey as string) || undefined,
			defaultModels: defaultModels as string[],
