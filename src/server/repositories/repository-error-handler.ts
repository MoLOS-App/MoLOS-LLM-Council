...
export function safeJsonParse<T>(
	...
	try {
		...
		console.error(`[RepositoryError] Failed to parse JSON${contextMsg}:`, error);
		if (error instanceof Error) {
			console.error(`[RepositoryError] JSON content:`, jsonString.substring(0, 200));
		}
		return fallback;
	}
}
...
export function handleDatabaseError(error: unknown, context: string): never {
	if (error instanceof Error) {
		...
		if (error.message.includes('SQLITE_BUSY')) {
			throw new RepositoryError(`Database is busy, please try again: ${context}`, error);
		}
	}

	console.error(`[RepositoryError] Unexpected error in ${context}:`, error);
	throw new RepositoryError(`An unexpected error occurred: ${context}`, error as Error);
}
