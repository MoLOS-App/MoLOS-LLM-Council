...
export class CouncilOrchestrator {
    ...
    try {
      ...
      for (const modelId of models) {
        const prompt = buildStage1Prompt(
          query,
          this.config.customPrompts?.stage1,
        );
      ...
      for (const modelId of models) {
        try {
          const rankings = await this.runStage2Model(
            query,
            stage1Responses,
            modelId,
          );
      ...
      try {
        const synthesisStream = this.runStage3Stream(
          query,
          stage1Responses,
          stage2Rankings,
        );
   ...
   */
  private async executeStage1Model(
    query: string,
    modelId: string,
  ...
  private async runStage2Model(
    ...
    const prompt = buildStage2Prompt(
      query,
      responsesForPrompt,
      this.config.customPrompts?.stage2,
    );
  ...
  async runCouncilSync(query: string): Promise<CouncilResult> {
    ...
    const stage1Promises = models.map((modelId) =>
      this.executeStage1Model(query, modelId),
    );
