...
export async function* orchestrateCouncil(
  ...
  const president = presidentPersonaId
    ? personas.find((p) => p.id === presidentPersonaId)
    : personas[0];
  ...
  onChunk({
    type: "meta",
    data: { personasCount: personas.length, presidentId: president.id },
  });
  ...
  const presidentProvider = await providerRepo.getById(
    president.providerId,
    userId,
  );
  ...
  onChunk({
    type: "done",
    data: { conversationId: convId, summary: fullSynthesis.slice(0, 500) },
  });
}
...
function buildSynthesisPrompt(responses: Map<string, string>): string {
  let prompt =
    "As the Council President, synthesize the following council responses into a comprehensive, balanced recommendation.\n\n";
...
function parseRankings(
  text: string,
): Array<{ personaId: string; rank: number; reason: string }> {
  try {
